"""Run the unified MILP storage+maint sweeps for all three benches (per backend).

Serial wrapper around scratch/measure_milp_curve.py; writes (per bench x kind):
  results/curves/{backend}/milp_storage_sgrid_<bench>.json
  results/curves/{backend}/milp_maint_sgrid_<bench>.json

Run detached:
  nohup .venv/bin/python -u scratch/measure_milp_curve_all.py \
      > scratch/logs_sgrid/milp_curve_all.log 2>&1 &
  # Oracle (storage only; maint + Oracle refuses in the runner):
  .venv/bin/python -u scratch/measure_milp_curve_all.py --backend oracle
"""
...
def main():
    ...
    for bench in ["census", "dmv", "stats_ceb_single"]:
        ...
        if backend != "oracle":          # maint+oracle refuses in the runner
            jobs.append((bench, "maint", MAINT_GRID))
    for bench, kind, grid in jobs:
        t0 = time.time()
        print(f"[run] {bench}/{kind} ({backend})", flush=True)
