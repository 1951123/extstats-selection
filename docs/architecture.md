...
> **状态：立场已拍板（软选择：乙+甲），契约设计进行中。** 本节记录 v2 的一个
> **核心架构矛盾**及其消解路径。它直接决定 `core/optimize.py` 是"单一共享 MILP"
> 还是需抽象成"可挑选优化器"，以及 backend 接口是否需为此增加"结构性质契约"。
...
- **共享层（优化模型的"类"统一）**：所有后端都落在 **MILP 这个类**内，但实例化时
  按契约选择具体的形式（稀疏线性 MILP / 通用乘性 MILP / min-max 线性化等）。
  这样"统一" = "所有后端都用 MILP 类 + 契约决定选哪种形式"，既比"每个 DBMS 一
  套优化器"统一，又比"所有后端用同一个精确模型"诚实。
- **后端层（结构性质契约）**：后端通过 `structural_props()` 暴露其估计模型与
  成本模型的性质，核心据此选择优化器。契约至少含四类维度：

  1. **稀疏性**（`sparse_one_stat: bool`）——该后端能否支持"每查询单统计即可捕获
     主导相关"，使目标退化为**精确线性**（§1.5）。PG：是（已实证）。Oracle：待验证。
     不满足 → 退回通用**乘性近似** MILP（仍是 MILP，但非同一简化形式）。
  2. **独立性/不相交**（`disjoint_supported: bool`）——能否通过"选出统计全局列
     不相交"保证联合效应独立（免 planner 干扰 / 使 $e_i(T_i)$ 合成可信）。PG：是
     （`global_disjoint` 实测预测比 1.000）。Oracle：机制不同，待验证。
  3. **成本结构**（`maint_structure ∈ {fixed+var, fixed_only, ...}`、`capacity_model`）
     ——维护成本是"固定+可加变动"还是"纯固定"，容量是"每统计"（PG target）还是
     "每表/每次采样"（Oracle estimate_percent）。这决定存储/维护预算约束的形式。
  4. **协议/测量**（`protocol_m: bool`，已有）——决定测量成本与可行性；间接影响
     能否廉价获得 query-level 预测量。

- **目标函数结构是独立于成本的契约维度。** 当前目标是最小化 workload 平均
  q-error，但"平均"只是多种聚合之一，聚合方式直接改变优化模型的可解性：
  ...
  - **最坏情况（min-max）**：$\min \max_i e_i$，需引入共享变量 $t$ 与逐查询约束
    $e_i \le t$，仍可线性化进 MILP，但结构不同。
  - **p 分位**：通常**不可**直接线性化，需要专门处理。
  因此目标**聚合方式**应作为 core/CLI 的模型选择参数（`objective='mean'|'geomean'|
  'worst'|'p90'`），同时后端通过契约声明"它在哪些聚合下能提供可信的估计"
  （`supports_objectives`），二者匹配才选用——**可解性由 core 优化器表达，后端
  可行性由契约表达**。

**校验（软选择语义）。** 核心在初始化时读取 `structural_props()` 与目标聚合选择，
为每个后端挑出"能支持的最强优化器"：

- 稀疏 + 不相交 + mean/geomean → **稀疏线性 MILP**（最快、共享）。
- 不支持稀疏但不相交 → **通用乘性 MILP**（同一类，不同形式）。
- min-max 目标 → **含 $t$ 变量的线性化 MILP**。
- 契约缺项或冲突 → 明确报错，迫使后端显式声明能力，而非静默用错模型。

**代价。** 统一性从"免费假设"变为"由契约校验保证的有条件统一"，并需为核心
`optimize` 与 backend 接口增加契约校验与多优化器选择。**已拍板立场**：软选择
（乙+甲）——**以乙为框架（契约 + 优化器选择抽象），以甲为默认（默认走共享稀疏
MILP）**，不满足契约的后端不是被拒绝，而是被分配到它能支持的最强优化器。
