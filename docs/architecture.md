...
> **状态：待拍板。** 本节记录 v2 的一个**核心架构矛盾**及其可能的消解路径。
> 它直接决定 `core/optimize.py` 是"单一共享 MILP"还是需抽象成"可挑选优化器"，
> 以及 backend 接口是否需为此增加"结构性质契约"。
...
**消解路径（结构性契约）。** 把"统一性"从*当然成立的 postulate* 降为*由后端
性质校验保证的有条件统一*：

- **共享层**：`core/optimize.py` 保持单一共享 MILP，前提是所有支持的 DBMS 都
  运行在"稀疏 + 列不相交"可信区（§1.5"用剪枝换可信度"的跨后端展开）。
- **后端层**：后端通过一个**结构性质契约**暴露其估计模型的性质
  （如 `structural_props()` 返回 `sparse_one_stat`、`disjoint_supported`、
  `maint_structure ∈ {fixed+var, fixed_only, ...}`、`capacity_model`）。
- **校验**：核心在初始化时检查契约——满足者走共享稀疏 MILP（快速、共享）；
  不满足者（若某 DBMS 无法保证独立性/稀疏性）**拒绝或降级**到更通用但更慢的
  求解路径。

**代价。** 这一方案使统一性从"免费假设"变为"由契约校验保证"，并需要为核心
`optimize` 与 backend 接口增加契约校验。**待决立场**：

- **立场甲（激进统一）**：坚信所有支持后端都能满足稀疏 + 不相交约定，保持单一
  共享 MILP，DBMS 差异全部压进后端参数。
- **立场乙（分层务实）**：承认部分后端可能无法满足简化约定，把"优化器选择"也
  抽象化，核心共享"可行性检查 + 元约束"，允许后端在 MILP 与更简单的特例间选择。
- 当前倾向：**以乙为框架，以甲为默认**——默认后端走共享稀疏 MILP，契约校验拦截
  不满足者。
