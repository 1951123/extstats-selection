...
> **〔修订 2026-09-04〕`per_query_cap=1`（one-stat）是"可选约束/精度档"，不是
> 通用模型的必选公理。** 新的裁决把"每查询最多选一个统计"从 §1.4 的"强特例、
> 模型结构性质"重定位为：**通用 what+how-much 优化问题允许每查询选 k≥0 个统计
> （cap 由后端能力或数据需要决定，可放开到 K）；cap=1 只是默认的
...
> - 但对"允许多统计 / 更高 arity"的后端或数据，cap 可上调 K（需整体建模叠加，
...
## 7bis. 采样优先容量 (S, param) 的优化模型（设计基准，未实现）
...
$$
S_t \;\ge\; 300\cdot\max_{s\,\in\,t} p_s \qquad\Longleftrightarrow\qquad \forall s\!\in\!t:\ p_s \le S_t/300,
$$

其中 $S_t$ 是表 $t$ 那次共享 ANALYZE 实际采的行数（`targrows`，封顶于 $N_t$）。**这条不等式用
"先定采样 $S_t$"来读，就自动把每个对象的表示参数限制在一个上界晶格里**：给定每表采样档 $S_t$，
可行 param 只有 $p_s\in[0,\,S_t/300]$。晶格之内 param 是自由的——更小的 param 由同一份深样本算
（free-rider、不被向下约束），**唯一耦合就是那个上界 cap $S_t/300$**。于是：
- **Direction B**（$p>S_t/300$）→ 由晶格**构造不可行**，无需 engine 特殊拒绝；
...
> PG 的 `S/300` cap**,把 `param` 读成随 λ 收窄。修正:Oracle 覆写 `max_param_at_level → None`(base
> docstring 本已写明"引擎不强制时(Oracle)返回 None")。
...
> 不是引擎约束。
> **〔表示轴定稿 2026-09-05〕Oracle 无 PG 式可调 per-object how-much 轴 → 保留通用 `candidates×params`
> 层,Oracle 的表示参数**退化单点 `(254,)`** = "让引擎自选桶数"。**理由**:PG 有 `statistics_target`
> 这种"逐对象"精度 knob,param grid 才能作为可优化 how-much 轴;Oracle 的直方图分辨率由引擎自决
> (`SIZE AUTO`/按 NDV 收敛到自然桶),`SIZE` 只是上界,实测(2026-09-05, L0)请求 64 vs 254 回落为
> 同量级少桶,不存在真实可调分辨率轴。因此 Oracle 侧不再是有多条 param 的 grid;`representation_param_tiers=(254,)`,
> 通用测量给每候选 `|param|=1`。之前"为让 Oracle 在 L0 测 64+254 而放开 cap"的动机被此定稿取代
> (Oracle 本就走单点 AUTO 语义;`max_param_at_level→None` 仍保留,含义=单点 254 永不被 PG 作风的
> cap 剪掉)。未定稿前的 "offer 64+254" 语料口径作废。
...
$$\min\ \underbrace{\tfrac1{|Q|}\sum_i \hat e_i^{(S_t,T_i)}}_{\text{query-level, 见 fidelity}}
\quad\text{s.t.}\quad
\begin{cases}
0\le p \le S_t/300,\\
\sum_p y_{C,p}\le 1,\\
...
$\lambda_{q}=\mathrm{truth}_q\cdot S_t/N_t$（$S_t$=该表在本 λ 档采的行数，见 §6.3f）。**不作为硬删**：
低 $\lambda_q$ 的查询不用乐观均值，而改用其在该采样档下的**保守估计**（如 `qerror_worst` / 上界 /
由 `qerror_std` 抬高——即用已记录的 §6.3f 字段），再进入上述 workload 平均。

**外层**——对每表采样档 $S_t\in\Lambda$ 各解一遍内层 MILP，比较时计入"这次部署的 fixed 扫描成本"，取：
$$\min_{S\in\Lambda}\ \Big[\ \text{innerObj}^{(S)}\ (\text{已含 query-level 保守化}) \
...
每个 S 档下对全部候选×param 预扫。**param 的上界被晶格 cap 切掉**：只预扫 $p\le S_t/300$（PG 把
越界组合本就不建的档跳过）；Oracle 无 engine 上界，若也要统计健全则把 $p\le S_t/300$ 作为可选护栏
施加。是否按 fidelity 边界(ω≈1/5)加密各 S 档留作数据驱动的后续开点，不硬编码 ladder。

> **同一表示参数须在它出现的每个 λ 下分别测量（不可跨 λ 复用读数）。** $(C,p)$ 的 q-error 记为
> $e_{i,(C,p)}^{(S_\lambda)}$，是 λ 的函数，不是只依赖 (C,p)：同一 param $p$ 在 λ₁ 浅采与 λ₂ 深采下，
> (i) 对象本身由不同样本算——驱动 combo 是否被捕获($\lambda_q$，§6.3f)随 $S$ 变化；(ii) 周围单列也在
> 该 λ 态($S_\lambda/300$)，规划器的整个估计上下文变。故测量网格是 **|Λ| × 候选 × 该 λ 下可容纳的
> param(≤`S_λ/300`)**：每个 (λ, 候选, 适配 param) 都要在该 λ 态里实测，不能"深 λ 测一次、浅 λ 复用"
> (那会把深采的捕获错误 credit 给浅部署)，也不能浅复用于深。唯一可跳过的情形是 $p>S_\lambda/300$
> (Direction B，该 λ 下不可建)。测量成本随 |Λ| 线性增长——这是采样优先的诚实代价，也是"为何要
> 离散少数 λ 档、不连续"的又一论据。

**骨架验证（PG，Census）**：对 3 条最坏相关查询的已知主导对，测其在各采样档 target
(=statistics_target，即 $S=300·\text{target}$) 下的 q-error 与每查询捕获 $\lambda_q$（PG 只用一个
`statistics_target`，故逐档读数即该档表现）：
...
读数：(1) **每查询 λ_q 越过 ~1 前后是 q-error 的剧变边界**（q.62 tgt50 λ.27→qerr45, tgt100 λ.55→2.07：
这正是 fidelity 阈值，跨引擎同构地可观测到）；(2) **大档单调性弱/回落**（q.62 在 2500 的 1.45 反而
到 10000 变 2.5），故外层需**搜索而非盲目抬采样 S**；(3) 诱饵列组各档都平(dDepart 系 ~3400)——"该 S 档
下哪个列组有效"在固定采样档时才是内层 MILP 要解决的可解释问题。证实 §7bis 的"先定每表采样 S、
param 活在其 $p\le S/300$ 上界内"结构在 PG 上行为合理；真正的实现仍在 §7bis 之外未做。

> **与 v1 的关系（殊途同归）：v1 ≡ 本模型限制到 λ=max。** v1 实质上把单列 target（=容量轴）定在
> 其最大可选（10000，近全表采样）作为基准，再在该采样所限的表示空间里选 ext；本模型的"单列 target
> $=S_\lambda/300$ 即 λ 的实现、并限制 ext param $\le S_\lambda/300$"这条链，在 λ=max 时退化为 v1。
> 即 **v1 是本模型 λ 取最大档的边界情形**（v1 从不把 λ 当变量、不量 per-tier baseline、不问"深采 vs
> 浅采"的取舍）。v2 的贡献是把 v1 隐含的 λ 轴显式化、做成可遍历的决策并用 per-λ 基线诚实权衡。
> 从相反直觉收敛到同一结构身份，是"此分解正确"的一处印证。
